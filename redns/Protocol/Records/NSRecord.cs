...
namespace redns.Protocol.Records
{
        ...
        : HostnameRecord
    {
        ...
        public NSRecord (Zone zone, string name, RecordClass @class, UInt32 ttl)
            : base (zone, name, @class, RecordType.NS, ttl)
        {
        }

        public override IEnumerable<ResourceRecordBase> GetResourceRecordsForQuery (Query query)
        {
            yield return new NSResourceRecord { Data = this.Hostname };
        }
    }
}
